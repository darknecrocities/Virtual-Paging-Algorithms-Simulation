    ...
    for i, page in enumerate(reference_string):
        ...
        if page in frames:
            ...
            if len(frames) < num_frames:
                frames.append(page)
                reference_bits.append(0)  # Initially 0 (Silberschatz: "all reference bits set to 0")
                explanation = f"Page {page} caused a fault. Added to an empty frame."
            else:
                # Replace logic
                while True:
                    if reference_bits[pointer] == 0:
                        # Replace this page (reference bit was 0 — no second chance)
                        old_page = frames[pointer]
                        frames[pointer] = page
                        reference_bits[pointer] = 0  # New page starts with ref bit 0
